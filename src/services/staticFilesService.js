...
async function generateStaticFiles(res) {
    ...
    resultSitemap = resultSitemap + await generateEntityFiles('state', stateModel);
...
async function generateSitemapIndex(sitemapFileName, sitemapFileUrl, itensQty) {
    ...
    const baseUrl = "https://www.cepsdobrasil.com.br/sitemap/";
    let xmlSitemapHeader = `
    <?xml version="1.0" encoding="UTF-8"?><?xml-stylesheet type="text/xsl" href="/assets/css/dbp-sitemap.xsl" ?>
...
async function generateSitemapUrls(sitemapType, sitemapBaseFileName, model, itensQty) {
    ...
    let xmlSitemapHeader = `
    <?xml version="1.0" encoding="UTF-8"?><?xml-stylesheet type="text/xsl" href="/css/dbp-sitemap.xsl" ?>
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        switch(sitemapType) {
            case 'state':
                ...
                break;
            case 'city':
                itens = await model.getAllCities(page, pageSize);
                break;
            case 'area':
                itens = await model.getAllAreas(page, pageSize);
                break;    
            case 'address':
                itens = await model.getAllAddress(page, pageSize);
                break;   
            case 'zip':
                itens = await model.getAllZip(page, pageSize);
                break;               
        }
        
        itens.forEach(item => {
            if (sitemapType == 'state') {
                let state = await model.getStateDetail();
